...
class MMConnection extends Node:
	...
	func send(sig, obj):
		socket.put_packet((sig + JSON.stringify(obj, "", false)).to_ascii_buffer())
...
class LobbyInfo extends Node:
	...
	static func new_host():
		...
		res.multiplayer.multiplayer_peer = res.net
	...
	static func new_client(code : String):
		...
		MMConnection._instance.send("CONN", { "code": code,  "port": int(addr[1]) })
	...
	func _on_client_requested(json : Dictionary):
		print_debug("handling client connection request (CLRQ)")
		if not is_host:
		...
		MMConnection._instance.send("CLOK", { "p": json.p, "port": int(addr[1]) })
...
static func connect_to_mm_server(port : int, username : String, logger = null):
	...
	var retry = 0
	while retry < 10:
		my_addr = await NetUtil.request_stun(sock, 2)
		if my_addr: break
		else:
			retry += 1
	...
	if not res.ok:
		...
		return
	logger.call("creating UDP connection...")
	...
	while true:
		...
		if retry > 10:
			...
			#maybe they didnt receive it, send again
			sock.put_packet('HELO'.to_ascii_buffer())
...
static func join(code, logger):
	if lobby:
		if logger: logger.call("lobby instance already exists!")
		return
	
	lobby = await LobbyInfo.new_client(code)
